...
class LeaveRecordResponse {
  ...
  LeaveRecordResponse.fromJson(Map<String, dynamic> json){
    ...
    data = List.from(json['data']).map((e)=>LeaveResponseModel.fromJson(e)).toList();
  }
...
class LeaveResponseModel {
  ...
  LeaveResponseModel.fromJson(Map<String, dynamic> json){
    if(json.containsKey('Leave'))
      Leave = List.from(json['Leave']).map((e)=>LeaveInfoModel.fromJson(e)).toList();
    else
      Leave = [];
    if(json.containsKey('Notification')) {
      Notification = List.from(json['Notification']).map((e) =>
          LeaveNotificationModel.fromJson(e)).toList();
    }else
      Notification = [];
  }
