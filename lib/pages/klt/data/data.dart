...
class NurseryVideo extends StatefulWidget {
  String className;
  String videoName;

  NurseryVideo({super.key, required this.className, required this.videoName});

  @override
  _KltHomePageState createState() => _KltHomePageState();
}

class _KltHomePageState extends State<NurseryVideo> {
  late List<KltVideoModel> specialities;
  final _className = 'Nursery';
    ...
    collectionRef.snapshots().listen((result) {
      for (var res in result.docChanges) {
        if (res.type == DocumentChangeType.added) {
          ...
          //debugPrint("removed");
          ////debugPrint(res.doc.data());
        } else if (res.type == DocumentChangeType.values) {
          //debugPrint("values ....");
          ////debugPrint(res.doc.data());
        }
      }
  ...
  Widget build(BuildContext context) {
    return getVideoWidgetFromListOfVideos(specialities, widget.videoName, 5);
  }
}
