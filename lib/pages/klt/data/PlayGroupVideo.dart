...
class PGVideo extends StatefulWidget {
  String className;
  String videoName;
  PGVideo({super.key, required this.className, required this.videoName});

  @override
  _KltHomePageState createState() => _KltHomePageState();
}

class _KltHomePageState extends State<PGVideo> {
  late List<KltVideoModel> specialities;
  final _className = 'PlayGroup';
        ...
        if (res.type == DocumentChangeType.added) {
          ...
          setState(() {
            if (res.doc.data().toString().contains('myvideo')) {
              ...
              /*      specialities.addAll(videoData.videoList.where((video) =>
                  video.classId == 6 &&
                  (widget.videoName.isEmpty ||
                      video.videoName
                          .replaceAll(" ", "")
                          .contains(widget.videoName.replaceAll(" ", ""))))); */
              debugPrint('Total videos found: ${specialities.length}');
            }
          });
  ...
  Widget build(BuildContext context) {
    return getVideoWidgetFromListOfVideos(specialities, widget.videoName, 6);
  }
}
...
Future<void> downloadFile(
  ...
  var storageReference =
      'gs://kidzeeandroidapp-91faf.appspot.com//klt_content/Videos/PlayGroup/${specialiti.videoName}.mp4';
  final storageRef = FirebaseStorage.instance.refFromURL(storageReference);

  //debugPrint(storageReference);
  //debugPrint("/klt_content/Videos/${specialiti.classId}/${specialiti.videoName}.mp4");
