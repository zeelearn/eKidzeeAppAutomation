...
class PGRhymesScreen extends StatefulWidget {
  String className;
  String videoName;
  PGRhymesScreen({super.key, required this.className, required this.videoName});

  @override
  _KltHomePageState createState() => _KltHomePageState();
}

class _KltHomePageState extends State<PGRhymesScreen> {
  ...
  void initState() {
    ...
    //debugPrint('QR CODE Video is ${widget.videoName} class : ${widget.className}');
    getRhymesInfo();
  }
                ...
                if (widget.className == 'Senior KG' &&
                  ...
                  if (widget.videoName.isNotEmpty) {
                    ...
                    if (rhymesList.isEmpty)
                      for (int jIndex = 0;
                          ...
                          jIndex++) {
                        ////debugPrint(rhymesData.videoList[jIndex].videoName);
                        if (rhymesData.videoList[jIndex].videoName
                            .contains(widget.videoName)) {
                          //debugPrint('video found for play PG 89');
                  ...
                  if (widget.videoName.isNotEmpty) {
                    ...
                    for (int jIndex = 0;
                      ...
                      if (rhymesData.videoList[jIndex].videoName
                          .contains(widget.videoName)) {
                        //debugPrint('video found for play PG 106');
                        rhymesList.add(rhymesData.videoList[jIndex]);
                      }
                    }
                  } else {
...
class RhymesAdaptar extends StatelessWidget {
  ...
  Future<void> downloadFile(
    ...
    var storageReference =
        'gs://kidzeeandroidapp-91faf.appspot.com//klt_content/Rhymes/PlayGroup/${specialiti.videoName}.$extention';
    ////debugPrint('gs://kidzeeandroidapp-91faf.appspot.com//klt_content/Rhymes/PlayGroup/${specialiti.videoName}.${extention}');
  ...
  Future<void> playVideo(
    ...
    if (filePath.existsSync()) {
      //debugPrint('Path Found play File PG 228');
      if (extention == 'mp3') {
        //debugPrint('mp3 player inilised240');
        debugPrint(
            '${appDocDir.path}/${videoModel.classId}/${videoModel.videoName.replaceAll(" ", "")}.$extention');
        debugPrint(videoModel.videoName);
        debugPrint(videoModel.videoUrl);
        debugPrint(videoModel.thumbanilUrl);
        Navigator.push(
          context,
          MaterialPageRoute(
        ...
        debugPrint(
            '${appDocDir.path}/${videoModel.classId}/${videoModel.videoName.replaceAll(" ", "")}.$extention');
        debugPrint(videoModel.toString());
        Navigator.push(
          context,
          MaterialPageRoute(
