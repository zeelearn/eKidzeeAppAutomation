...
class PentemindActivityScreen extends StatefulWidget {
  String contentType;
  PentemindActivityScreen({super.key, required this.contentType});

  @override
  _LearningMaterialScreenState createState() => _LearningMaterialScreenState();
}

class _LearningMaterialScreenState extends State<PentemindActivityScreen>
        ...
        child: Column(
          children: [
            Flexible(
              ...
              itemCount: mMaterials.length,
              shrinkWrap: true,
              itemBuilder: (context, index) {
                return getActivityWidget(mMaterials[index], index);
              },
            ))
          ],
        ),
  ...
  getIcon(LearningMaterialModel model, int index) {
    return GestureDetector(
      onTap: () {
        ...
        Utility.downloadFile(model.WebUrl, '${model.ContentDescription}.pdf')
            .then((value) {
          isLoading = false;
          getMaterials();
        });
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          color: imageList.length > index && imageList[index]
