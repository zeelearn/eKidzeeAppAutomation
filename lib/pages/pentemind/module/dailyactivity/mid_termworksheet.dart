...
class MidTermWorksheetScreen extends StatefulWidget {
  const MidTermWorksheetScreen({super.key});

  @override
  _WorksheetScreenState createState() => _WorksheetScreenState();
}

class _WorksheetScreenState extends State<MidTermWorksheetScreen>
...
//     debugPrint('WorksheetScreen Screen didChangeAppLifecycleState ${state} ');
  ...
  getActivities() {
    ...
    apiService
        .getDailyMidTermActivity(request, _dayController.text.toString(), token)
        .then((value) {
      if (value != null) {
        isLoading = false;
        if (value == null) {
          ...
          mActivity.sort((a, b) => a.Worksheet.compareTo(b.Worksheet));
          setState(() {});
        } else {
          Utility.showMessage(context, 'data not found');
        }
  ...
  Widget build(BuildContext context) {
    FirebaseAnalyticsUtils()
        .sendAnalyticsEvent('DailyActivity:MidTermWorksheetScreen');
    return Scaffold(
  ...
  getActivityWidget(DailyActivityModel model) {
    return GestureDetector(
      onTap: () {
        if (true || model.LogBookStatusCode == 'C') {
          Navigator.push(
            context,
            MaterialPageRoute(
