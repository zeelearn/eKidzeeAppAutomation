...
mixin ListScrollMixin<T extends StatefulWidget> on State<T>
  ...
  void _handleScroll() {
    ...
    if (maxScroll - currentScroll <= _scrollDelta && canFetch) {
      fetch();
    } else if (currentScroll < _refreshTrigger && canRefresh) {
      canRefresh = false;
      fetch(0);
...
// class _MyScreenState extends State<MyScreen> with ListScrollMixin {
  ...
  // void fetch([int offset]) {
    // debugPrint('Fetching items, starting at ${offset ?? dummyWinesData.data.length}');
    // ...
