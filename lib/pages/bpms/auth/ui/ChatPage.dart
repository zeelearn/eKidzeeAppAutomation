...
import 'package:ekidzee/api/request/pentemind/file_upload.dart';
import 'package:ekidzee/helper/LocalConstant.dart';
import 'package:ekidzee/helper/math_utils.dart';
import 'package:ekidzee/pages/bpms/auth/data/enums/auth_status.dart';
import 'package:ekidzee/pages/centersetup/chat/utils/CameraScreen.dart';
...
import '../../../centersetup/CustomUI/OwnMessgaeCrad.dart';
import '../../../centersetup/Model/MessageModel.dart';
...
class ChatPage extends StatefulHookConsumerWidget {
  ...
  ChatPage({super.key, required this.taskModel});
...
class _ChatPageState extends ConsumerState<ChatPage>
  ...
  initData() async {
    //await getUserInfo();
    imgList.clear();
//     debugPrint('chat initData-------------------------');
    if (widget.taskModel.files.isNotEmpty) {
//       debugPrint('initData---${widget.taskModel.files}');
...
//     debugPrint('length ${messages.length}');
  ...
  Widget build(BuildContext context) {
    final auth = ref.watch(authNotifierProvider);
//     debugPrint("build isLoading ${auth.loading} ");
    if (auth.loading == AuthStatus.authenticated) {
//       debugPrint('loading status false');
      isLoading = false;
    }
...
//       debugPrint('afterFileUploading 384');
      //ref.read(authNotifierProvider.notifier).checkAuthStatus();
      setState(() {
        isLoading = false;
      });
  ...
  chatView(FranchiseeInfoModel model) {
    return Stack(
      children: [
        Image.asset(
        ...
        Scaffold(
          ...
          appBar: AppBar(
            ...
            actions:
                widget.taskModel.statusname.toLowerCase().contains('progres')
                    ? [
                        InkWell(
                          onTap: () {
                          ...
                          builder: (context, snapshot) {
                            if (!snapshot.hasData) {
                              return const SizedBox.shrink();
                                  ...
                                  debugPrint(
                                      'Response from background is - $data');

                                  if (data.containsKey('success')) {
                                    UploadImageResponse value =
                                        UploadImageResponse.fromJson(jsonDecode(
                                    ...
                                    WidgetsBinding.instance
                                      ...
                                      Utility.showMessageCallback(context,
                                          'Alert', data['error'], this);
                                    });
                                  }
...
//     debugPrint('in BP COmpltedadadakldnakldn123 $value $_status');
...
//           debugPrint('image update ${response.taskDetail[index].title}');
        }
      }
...
//     debugPrint('upload image $value');
    DBHelper dbHelper = DBHelper();
    dbHelper.insertSyncData(
        FileUploadModel(
          mTaskId: widget.taskModel.id,
          path: value,
          userId: userId,
          request: Utility.getFileName(value),
        ).toJson(),
...
//       debugPrint('ACTION_IMAGE_UPLOAD_RESPONSE_ERROR');
      setState(() {
        isLoading = false;
      });
    } else if (action == Utility.ACTION_IMAGE_UPLOAD_RESPONSE_OK) {
//       debugPrint('ACTION_IMAGE_UPLOAD_RESPONSE_OK');
      setState(() {
        isLoading = false;
      });
...
//           debugPrint('1297 image Response');
          //ref.read(authNotifierProvider.notifier).checkAuthStatus();
        } else {
